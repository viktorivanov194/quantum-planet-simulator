...
from app.services.planet_service import generate_planet_profile_and_state, validate_planet_profile
...
def run_simulation_pipeline(request: SimulationRunRequest) -> SimulationRunResponse:
    ...
    profile, state = generate_planet_profile_and_state(
    ...
    chemistry = get_candidate_molecules(
        CandidateRequest(profile=profile, state=state, validation=validation, max_candidates=request.max_candidates)
    )
    ...
    if selected_candidate is not None:
        ...
        spectrum = generate_synthetic_spectrum(
            SpectrumRequest(
                ...
                abundance_proxies=chemistry.abundance_proxies,
                quantum_result=None,
                chemistry_candidates=quantum_candidates,
                scientific_profile=scientific_proxy_profile,
            )
        )
        molecular_probe_response = run_molecular_probe(
            build_molecular_probe_request(
                candidates=quantum_candidates,
                runtime_mode={"demo_balanced": "auto", "cached_only": "cached_only", "fallback_only": "disabled"}.get(
                    request.quantum_runtime_mode,
                    "auto",
                ),
                selected_formula=selected_candidate.formula,
            )
        )
        molecular_probe = molecular_probe_response.selected_result
        ...
        final_report = build_final_report(
            ...
            molecular_probe=molecular_probe,
            spectrum=spectrum,
