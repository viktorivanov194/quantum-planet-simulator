...
import { simulationPresets, stageDefinitions } from "@/lib/config/presets";
import { PresetKey, QualityMode, SimulationResponse, StageId } from "@/lib/types/simulation";
...
export function SimulationShell() {
  ...
  const visualProfile = result?.visual_physics_profile;
  ...
  const effectiveStage = presentationStage ?? activeStage;
  ...
  const launchSimulation = async () => {
    ...
    try {
      const requestPromise = runSimulation(selectedPreset.payload);

      for (const [index, stage] of stageOrder.entries()) {
        setActiveStage(stage);
        await wait(index === stageOrder.length - 1 ? 560 : 460);
      }
  ...
  useEffect(() => {
    ...
    const timers = stageOrder.slice(1).map((stage, index) =>
      window.setTimeout(() => {
  ...
  return (
    <main className="relative h-screen overflow-hidden bg-space-shell">
      <PlanetCanvas
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_55%,transparent_0%,rgba(2,6,23,0.18)_42%,rgba(2,6,23,0.8)_100%)]" />
      ...
      <div className="relative z-10 flex h-full flex-col justify-between p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid items-end gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="pointer-events-auto justify-self-end lg:w-full lg:max-w-md">
        ...
        <div className="flex items-end justify-between gap-4">
          <div className="hidden max-w-3xl lg:block">
            <StageStrip stages={stageDefinitions} activeStage={effectiveStage} />
          </div>
          ...
          <div className="pointer-events-auto w-full max-w-lg">
            <AnimatePresence mode="wait">
              {error ? (
                ...
                <ResultOverlay key="result" result={result} stage={presentationStage ?? "planet-birth"} />
              ) : (
                <OverlayCard
                  key="standby"
        ...
        <div className="pointer-events-none absolute bottom-3 left-1/2 hidden w-[min(1080px,calc(100vw-2rem))] -translate-x-1/2 lg:block">
...
function ResultOverlay({ result, stage }: { result: SimulationResponse; stage: StageId }) {
  ...
  return (
    <div className="space-y-4">
      <AnimatePresence>
        {isDiscoveryStage ? (
          <motion.section
            ...
            exit={{ opacity: 0, y: 16 }}
            className={[
              "overlay-shell relative overflow-hidden p-5 sm:p-6",
      ...
      <PlanetStageExperience result={result} stage={stage} />
    </div>
  );
}

function PlanetStageExperience({ result, stage }: { result: SimulationResponse; stage: StageId }) {
  return (
    <>
      {(stage === "atmospheric-validation" || stage === "chemistry-emergence" || stage === "quantum-evaluation" || stage === "spectrum-reveal" || stage === "final-discovery") ? (
        <AtmosphereOverlay result={result} />
      ) : null}
      {(stage === "chemistry-emergence" || stage === "quantum-evaluation" || stage === "spectrum-reveal" || stage === "final-discovery") ? (
        <ChemistryOrbitOverlay result={result} />
      ) : null}
      {(stage === "quantum-evaluation" || stage === "spectrum-reveal" || stage === "final-discovery") ? (
        <QuantumChamberOverlay result={result} />
      ) : null}
      {(stage === "spectrum-reveal" || stage === "final-discovery") ? <SpectrumOverlay result={result} /> : null}
    </>
  );
}

function AtmosphereOverlay({ result }: { result: SimulationResponse }) {
  ...
  return (
    <motion.section
      ...
      className="pointer-events-auto absolute left-6 top-[24%] hidden w-[290px] lg:block"
    >
      <div
        className={[
          "overlay-shell p-5",
...
function ChemistryOrbitOverlay({ result }: { result: SimulationResponse }) {
  const visual = result.visual_physics_profile;
  const scientific = result.scientific_proxy_profile;
  const candidates = result.chemistry.candidates.slice(0, 5);
    ...
    <div className="pointer-events-none absolute inset-0 hidden lg:block">
      {candidates.map((candidate, index) => (
        <motion.div
          ...
          animate={{
            opacity: 1,
            scale: selected.has(candidate.formula) ? 1.06 : 1,
        ...
        >
          ...
          >
            <div className="text-[10px] tracking-[0.22em] text-slate-400">
              {selected.has(candidate.formula) ? "Locked Candidate" : "Candidate"}
            </div>
            <div className="mt-1 text-lg font-semibold text-white">{candidate.formula}</div>
            <div className="mt-1 text-[10px] uppercase tracking-[0.22em] text-slate-500">{candidate.tag}</div>
          </div>
        </motion.div>
      ))}
      <div className="pointer-events-auto absolute right-6 top-[58%] hidden w-[260px] xl:block">
...
function QuantumChamberOverlay({ result }: { result: SimulationResponse }) {
  const visual = result.visual_physics_profile;
  ...
  const sourceTone = source === "live" ? "#86efac" : source === "cached" ? visual.atmosphere_glow_color : "#fbbf24";
  ...
  return (
    <div className="pointer-events-none absolute inset-0 hidden lg:block">
      ...
      <motion.section
        ...
        className="pointer-events-auto absolute left-1/2 top-[58%] w-[320px] -translate-x-1/2 -translate-y-1/2"
      >
        <div className="overlay-shell p-5 text-center" style={{ boxShadow: `0 0 ${26 + visual.quantum_chamber_intensity * 30}px ${hexToRgba(sourceTone, 0.22)}` }}>
          ...
          <div className="mt-5 space-y-4 text-left">
            <GaugeBar label="Stability" value={stability} accent={sourceTone} />
            <GaugeBar label="Confidence" value={confidence} accent={sourceTone} />
          </div>
        </div>
      </motion.section>
    </div>
...
function SpectrumOverlay({ result }: { result: SimulationResponse }) {
  if (!result.spectrum) {
    return null;
  }
  ...
  return (
    <motion.section
      ...
      animate={{ opacity: 1, y: 0 }}
      className="pointer-events-auto absolute bottom-6 left-1/2 w-[min(980px,calc(100vw-2rem))] -translate-x-1/2"
    >
      <div className="overlay-shell overflow-hidden p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
        ...
        <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-black/35 p-4">
          ...
          <svg viewBox={`0 0 ${width} ${height}`} className="h-[180px] w-full">
            ...
            <polygon points={area} fill="url(#spectrumFill)" />
            <polyline
              fill="none"
              ...
              points={points}
            />
            {result.spectrum.highlighted_features.slice(0, 4).map((feature) => {
